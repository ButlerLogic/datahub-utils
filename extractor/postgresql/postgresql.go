...
func mapEntityToDoc(record map[string]interface{}, document *doc.Doc) error {
	...
	// if forceBool(record["primary_key"]) && record["key"] != nil && record["key"].(bool) {
	// 	keytype := strings.Replace(forceString(record["key_type"], "foreign"), " key", "", 1)
	// 	if forceBool(record["primary_key"], false) {
	// 		keytype = "primary"
	// 	}

	// 	set.UpsertKey(&doc.Key{
	// 		Name:  forceString(record["key_name"]),
	// 		Type:  keytype,
	// 		Items: []string{item.FQDN},
	// 	})
	// }
